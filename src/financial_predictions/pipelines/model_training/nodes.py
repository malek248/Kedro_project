...
def find_best_weight(
    X_train: np.ndarray, y_train: np.ndarray, weights: List[int], cv: int
) -> int:
...
def train_initial_model(
    X_train: np.ndarray, y_train: np.ndarray, best_weight: int, xgb_base: Dict
) -> XGBClassifier:
...
def transform_train(selector: SelectFromModel, X_train: np.ndarray) -> np.ndarray:
    """Apply your selector to the training set."""
    return selector.transform(X_train)


def transform_test(
    selector: SelectFromModel, X_test: np.ndarray
...
def train_final_model(
    X_train_sel: np.ndarray, y_train: np.ndarray, best_weight: int, xgb_base: Dict
) -> XGBClassifier:
...
def predict_model(
    model: XGBClassifier, X_test_sel: np.ndarray
) -> Tuple[List, List]:  # Changed y_proba return type hint to List
